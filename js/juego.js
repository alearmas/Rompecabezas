...
var instrucciones = ["Usar las flechas para desplazar los cuadros,","Mover los cuadros en cualquier dirección,", "Hacer que la imagen sea igual al objetivo que se muestra a la izquierda."];
...
function chequearSiGano() {
    ...
    for (var i = 0; i < 3; i++) {
        for (var j = 0; j < 3; j++) {
            if (grilla[i][j] == winnerGrid[i][j]) {
                acumulador++;
            }       
        }
    }
    return (acumulador.length === 9);
}
...
function actualizarUltimoMovimiento(direccion) {
  direccion = document.getElementById('flecha');
  switch (direccion) {
